...
const LastPiecesPage = () => {
  ...
  async function getLastPiecesPage() {
    const response = await axios.get(
      `${baseUrl}/products/last-pieces?page=${toPage}`,
      {
        headers: {
          "X-API-KEY": XApiKey,
        },
  ...
  let { data, isError, error, isLoading, isFetching } = useQuery({
    queryKey: ["LastPiecesPage", toPage],
    queryFn: getLastPiecesPage,
  });
      ...
      <div className="pt-30">
        <div className="flex justify-center py-5">
          <h3 className="text-3xl font-bold flex items-center">
            <FaPuzzlePiece size={35} className="mx-3" /> {t('lastPieces.title')}
          </h3>
        </div>
